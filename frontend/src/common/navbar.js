...
import { navigation } from '../data/acoountLinks';
...
const Navbar = () => {
  ...
  const navbarClass =
    scrollPosition > 50 ? 'bg-white text-[#4CAF50]' : 'bg-transparent text-white';

  useEffect(() => {
    const storedLoggedIn = Boolean(jwt) && Object.keys(userData).length !== 0;
    setLoggedIn(storedLoggedIn);
  }, [jwt, userData]);

  return (
    <div
      className={`fixed overflow-hidden flex justify-between w-full h-20 px-4 md:px-12 z-50 transition-colors duration-300 ${navbarClass}`}
    >
          ...
          <ul className="hidden md:flex">
            {loggedIn &&
              links.map(({ id, link }) => (
                <li
                  key={id}
                  className={`px-9 font-medium cursor-pointer hover:text-green-700 font-medium${navbarClass}`}
                >
                  <Link
                    activeClass="active"
                    to={link}
          ...
          {loggedIn ? (
            <ul className="list-none">
              <li className="flex justify-between items-center px-3 cursor-pointer font-medium tracking-wider">
                ...
                >
                  Sign out
                  <span className="absolute bottom-10 left-1/2 transform -translate-x-1/2 scale-0 rounded bg-blue-100 p-2 text-xs text-gray-600 group-hover:scale-100">
                    Sign Out
                  </span>
                </button>
              </li>
            ...
            <ul className="list-none flex justify-between items-center">
              <li className="px-3 cursor-pointer font-medium tracking-wider hover:scale-100 hover:text-gray-700">
                <button
                  className={`uppercase text-sm flex justify-center items-center ${navbarClass}`}
                  onClick={handleLogin}
                >
